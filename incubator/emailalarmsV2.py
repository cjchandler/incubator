...
class alarm:
    ...
    def sound_alarm_if_applicable(self):
        if self.is_alarming == True: # is alarming?
            ...
            print( "alarming on " + self.incubator_name + " " + self.alarm_name + " " + str(self.value))
            ...
            if time.time() - self.last_alarm_sent > self.send_interval:
                ...
                if self.last_hush + 60*60*self.last_hush_hrs > time.time():
                    ...
                    print("   no alarm sent, it's hushed until stamp" + str(self.last_hush + 60*60*self.last_hush_hrs))
                    pass 
                else: #ok we are in an alarms state, it's been enough time since the last alarm, send it! 
                    ES.send_email( str(self.incubator_name) + " " + str(self.alarm_name) + " Value: " + str(self.value) , "error at:" +str(time.ctime()) + "Reply a number to hush for that many hours (not working yet)"  ) 
                    print("     sent alarm via email")
                    self.last_alarm_sent = time.time()
            
            else: 
                print( "    it's alarming, but we already sent a message recently")
...
class alarm_loop:
    ...
    def __init__(self, incubatorname , test_turning_init):
        self.test_turning = test_turning_init
        self.incubator_name = incubatorname
        self.today_filename = "today_data"+self.incubator_name+".csv"
        self.df_now = pd.read_csv("./"+ self.today_filename)
        
        
        self.temp_alarm = alarm()
        self.temp_alarm.alarm_name = "temperature_1_C" 
        self.temp_alarm.incubator_name = self.incubator_name 
        
        self.humidity_alarm = alarm()
        self.humidity_alarm.alarm_name = "humidity_1" 
        self.humidity_alarm.incubator_name = self.incubator_name 

        
        self.turning_alarm = alarm()
        self.turning_alarm.alarm_name = "turning"
        self.turning_alarm.incubator_name = self.incubator_name 
        ...
        self.logging_alarm.incubator_name = self.incubator_name 
    ...
    def update_alarm_value_from_file(self):
        ...
        if secs_data > 2*60*60:
            print( " not enough data for turning verification")
            self.turning_alarm.value = 0.5
        
        ##update the alarms states (alarming on not etc) 
        if( self.test_turning== True):
...
while True: 
    ...
    print("incubator email alarms" )
    print( AL.incubator_name)
    print( AL.temp_alarm.value)
    print( "seconds since last save" , AL.logging_alarm.value)
    print( "turning value" , AL.turning_alarm.value)
    print( "humidity" , AL.humidity_alarm.value)
    time.sleep(10)
